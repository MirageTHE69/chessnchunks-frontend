...
const CommunicationCoach: React.FC = () => {
  ...
  const [unreadCount] = useState(3); // Example unread message count

  useEffect(() => {
    const fetchBatches = async () => {
      setBatches([
        { label: "Batch A", value: "batch_a" },
        { label: "Batch B", value: "batch_b" },
      ]);
    };
    const fetchStudents = async () => {
      setStudents([
        ...
        {
          label: "magnus_carlson",
          value: "magnus_carlson",
          image: "image.png",
        },
      ]);
    };
        ...
        <ul>
          {students.map((student) => (
            <li
              key={student.value}
              className="flex items-center p-2 cursor-pointer hover:bg-gray-700"
              onClick={() => handleUserClick(student.label)}
            >
              <div className="w-8 h-8 rounded-full bg-gray-600 flex items-center justify-center mr-2">
                <Image
                  src={
                    student.image ? `/${student.image}` : "/fallback-image.png"
                  }
                  alt={student.label}
                  width={32}
                  height={32}
      ...
      <div className="w-3/4 p-6 bg-gray-800 text-white flex flex-col">
        ...
        >
          {messages.map((msg, index) => (
            ...
            >
              ...
              <div
                className={`bg-${
                  msg.sender === conversationUser ? "blue-600" : "gray-300"
                } text-${
        ...
        <div className="my-6">
          ...
          <Select
            ...
            onChange={(selected) =>
              setSelectedBatches(selected as OptionType[])
            }
        ...
        <div className="my-6">
          ...
          <Select
            ...
            onChange={(selected) =>
              setSelectedStudents(selected as OptionType[])
            }
        ...
        <div className="flex flex-col mb-4">
          {" "}
          {/* Make it a flex column to stack editor and button */}
          <div className="mb-2">
            {" "}
